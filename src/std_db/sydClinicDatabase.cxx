...
// --------------------------------------------------------------------
void syd::ClinicDatabase::CreateTables()
{
...
// --------------------------------------------------------------------
bool syd::ClinicDatabase::QueryPatientByNameOrStudyId(const std::string & arg,
...
// --------------------------------------------------------------------
void syd::ClinicDatabase::QueryPatientsByNameOrStudyId(const std::string & arg,
...
// --------------------------------------------------------------------
bool syd::ClinicDatabase::QueryInjectionByNameOrId(const Patient & patient,
...
// --------------------------------------------------------------------
syd::TableElement * syd::ClinicDatabase::InsertFromArg(const std::string & table_name,
...
// --------------------------------------------------------------------
syd::Injection * syd::ClinicDatabase::InsertInjection(std::vector<std::string> & arg)
{
  if (arg.size() < 4) {
...
// --------------------------------------------------------------------
void syd::ClinicDatabase::DumpDicom(std::ostream & os,
                                    syd::Patient & patient,
...
// --------------------------------------------------------------------
void syd::ClinicDatabase::DumpPatients(const std::vector<std::string> & query,
                                       std::ostream & os)
{
  DD("DumpPatient");
  std::string patientname;
  if (query.size() == 0) patientname = "";
  else patientname = query[0];
  std::vector<Patient> patients;
  QueryPatientsByName(patientname, patients);

  PrintTable table;
  table.AddColumn("#sid");
  table.AddColumn("name");
  table.AddColumn("w(kg)");
  table.AddColumn("id");
  table.AddColumn("111I",18);
  table.AddColumn("MBq",6,1);
  table.AddColumn("90Y",18);
  table.AddColumn("MBq",12,1);
  table.Init();

  for(auto p:patients) {
    table << p.study_id << p.name << p.weight_in_kg << p.id;

    // Get Indium injection
    std::vector<Injection> injections;

    Query<Injection>(odb::query<Injection>::patient->id == p.id and // FIXME
                     odb::query<Injection>::radionuclide == "Indium111", injections);
    if (injections.size() == 0) table << "no" << 0.0 ;
    if (injections.size() == 1) table << injections[0].date
                                      << injections[0].activity_in_MBq;
    if (injections.size() > 1) table << injections.size() << "-";

    // Get Yttrium injection
    injections.clear();
    Query<Injection>(odb::query<Injection>::patient->id == p.id and // FIXME
                     odb::query<Injection>::radionuclide == "Yttrium90", injections);
    if (injections.size() == 0) table << "no" << 0.0;
    if (injections.size() == 1) table << injections[0].date
                                      << syd::ToString(injections[0].activity_in_MBq);
    if (injections.size() > 1) {
      std::string s;
      for(auto i:injections) s+=ToString(i.activity_in_MBq)+"+";
      s.pop_back(); // remove trailing '+'
      table << injections.size() << s;
    }
  }
  table.Print(os);
}
...
// --------------------------------------------------------------------
void syd::ClinicDatabase::DumpSeries(const std::vector<std::string> & query,
                                     std::ostream & os)
{
  std::string patientname;
  if (query.size() == 0) patientname = "";
  ...
  table.AddColumn("#sid");
  table.AddColumn("name");
  table.AddColumn("nb",4); // nb of series
  table.AddColumn("mod",20); // list of modalities (1 CT 3 NM 4 OT)
  table.Init();

  for(auto p:patients) {
    table << p.study_id << p.name;
    // Get list of series
    std::vector<DicomSerie> series;
    Query<DicomSerie>(odb::query<DicomSerie>::patient->id == p.id, series);
    table << series.size();
    // Get list of modalities
    std::map<std::string,int> list;
    for(auto s:series) {
      if (list.find(s.dicom_modality) != list.end()) {
        list[s.dicom_modality]++;
      }
      else list[s.dicom_modality] = 1;
    }
    std::string mod;
    for(auto l=list.begin(); l != list.end(); l++) {
      mod += syd::ToString(l->second)+" "+l->first+" ";
    }
    table << mod;
  }
  table.Print(os);
...
// --------------------------------------------------------------------
void syd::ClinicDatabase::QueryPatientsByName(const std::string & pattern,
                                              std::vector<Patient> & patients)
{
  if (pattern == "all" or pattern == "") {
    Query<Patient>(patients);
  }
  else {
    std::vector<std::string> names;
    syd::GetWords(pattern, names);
    //Query<Patient>(odb::query<Patient>::name.in_range(names.begin(), names.end()), patients);
    // We perform a word by word search, allowing pattern in the query "%"
    for(auto s:names) {
      std::vector<Patient> p;
      Query<Patient>(odb::query<Patient>::name.like(s), p);
      for(auto ip:p) patients.push_back(ip);
    }
  }
  // Sort by study_id
  std::sort(begin(patients), end(patients),
            [this](Patient a, Patient b) { return a.study_id < b.study_id; }  );
}
// --------------------------------------------------------------------


// --------------------------------------------------------------------
std::string syd::ClinicDatabase::GetAbsoluteFolder(const DicomSerie & serie)
{
...
// --------------------------------------------------------------------
std::string syd::ClinicDatabase::GetAbsoluteFolder(const Patient & patient)
{
...
// --------------------------------------------------------------------
std::string syd::ClinicDatabase::GetPatientPath(IdType patient_id_fk)
{
